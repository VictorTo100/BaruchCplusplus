...
namespace VISHRUTTALEKAR
{
	...
	namespace CAD
	{
		...
		*/
		Circle::Circle(const Circle& source_c) : Shape(), m_center(source_c.m_center), m_radius(source_c.m_radius)
		{
		...
		*/
		...
		string Circle::ToString() const
		{
			...
			*/
			/*Add the call to the ToString function in base(Shape) class
			*/
			std::string s = Shape::ToString();
			output << fixed << showpoint << setprecision(2)
				<< "The Center of the Circle is at:" << m_center.ToString() << "and the Radius is:" << Radius() << " The id of Circle is: " << s << endl;
